...
void busca_maior_folha(Node *root);
...
int main(void)
{
    ...
    while(op!=0)
    {
        ...
        printf("6 - Exibe o maior valor entres todas as folhas da árvore:\n");
...
void busca_maior_folha(Node *root)
{
    if(root!= NULL)
    {
        if(root->left == NULL && root->right == NULL)
        {
            if(root->item > maior_valor)
            {
                maior_valor = root->item;
            }
            
        }else{
            busca_maior_folha(root->left);
            busca_maior_folha(root->right);
        }
    }
